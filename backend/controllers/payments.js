...
exports.getPurchaseHistory = async (req, res) => {
    try {
        ...
        // Find orders for the user and populate course and user details
        const orders = await Order.find({ user: userId })
            .populate({
                path: 'course',
                model: 'Course',
                select: 'courseName price description thumbnail instructor',
                populate: {
                    path: 'instructor',
                    model: 'User',
        ...
        // Transform the orders data to include purchase details
        const purchaseHistory = orders.map(order => ({
            _id: order._id,
            orderId: order._id,
            courseName: order.course?.courseName || 'N/A',
            courseDescription: order.course?.description || 'N/A',
            thumbnail: order.course?.thumbnail || '',
            price: order.amount || 0,
            purchaseDate: order.purchaseDate,
            status: order.status ? "Completed" : "Inactive",
            transactionId: order.transactionId,
            paymentMethod: order.paymentMethod,
            // Include full order details for invoice generation
            orderDetails: {
                _id: order._id,
                user: order.user,
                course: order.course,
